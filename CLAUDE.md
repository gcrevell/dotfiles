...
6. Fills in missing keys from the tracked `claude-settings.json` into `~/.claude/settings.json` (via a small `python3` snippet) — it only ever *adds* keys that aren't already there; anything already present, at any nesting level, is left untouched.
...
Similarly, **`claude-settings.json` is the source of truth for Claude Code defaults across machines, but local always wins.** Claude Code itself has no `~/.claude/settings.local.json` layering for user-level settings (unlike project scope), so `install.sh` implements "local wins" itself: it only fills in keys missing from `~/.claude/settings.json`, never overwrites a key that's already set there — whether you set it by hand or a previous install run wrote it. This means bumping a value in `claude-settings.json` will only reach machines where that key isn't already present; to actually change an already-set value on a given machine, edit `~/.claude/settings.json` there directly (or delete the key and rerun `install.sh` to pick up the new default).
