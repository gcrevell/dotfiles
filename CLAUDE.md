...
6. Within `mac-install.sh`, under `personal` only, sets up Claude Code token-usage shipping: clones `github.com/gcrevell/claude-token-influx` to `~/src/claude-token-influx` (fast-forward only if already present), installs `claude-tokens-influx.sh` to `~/.local/bin/claude-tokens-influx`, renders `claude-tokens.plist` into `~/Library/LaunchAgents/com.skyler.claude-tokens.plist`, and `bootout`s then `bootstrap`s the agent. See below.
...
**All configuration is untracked, in `~/.config/claude-tokens/config.vars`** — `CLAUDE_TOKENS_INFLUX_URL`, `_TOKEN`, `_ORG`, `_BUCKET`, `CLAUDE_TOKENS_HOST`, and the optional `CLAUDE_TOKENS_REPO_DIR`. The wrapper is a **zsh** script specifically so it can `source` that file. This is a dedicated file holding nothing but these assignments, deliberately not `~/.zshrc.local`, which is free to carry arbitrary shell logic that has no business running inside a launchd job. None of these get defaults except `CLAUDE_TOKENS_REPO_DIR` (`~/src/claude-token-influx`): this repo is public, so any default for the rest would commit a hostname or bucket name, and a wrong `CLAUDE_TOKENS_HOST` silently fragments the InfluxDB series rather than failing.
