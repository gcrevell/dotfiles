...
# ---------------------------------------------------------------------------
# Claude Code token usage -> InfluxDB (macOS + personal only)
#
...
# POST the result.
#
# macOS + personal only. The headless Pi fleet ships the same measurement a
# different way (Telegraf's exec input, configured by home-ansible), and
# running both on one machine would write the same series twice.
# ---------------------------------------------------------------------------
if [[ "$(uname -s)" == "Darwin" && "$ENVIRONMENT" == "personal" ]]; then
  ...
  # The token lives in ~/.zshrc.local, so tighten it whether or not it's
  # configured yet — better to fix the mode before the secret arrives.
  chmod 600 "$LOCAL_ZSHRC" 2>/dev/null || true
  ...
  # The agent is useless until ~/.zshrc.local names a destination, and RunAtLoad
  # means it has already failed once by now. Point at the log rather than
  # letting it fail invisibly every hour.
  if ! grep -q 'CLAUDE_TOKENS_INFLUX_URL' "$LOCAL_ZSHRC" 2>/dev/null; then
    warn "claude-tokens is installed but not configured. Add to ~/.zshrc.local:"
    warn "  export CLAUDE_TOKENS_INFLUX_URL=... CLAUDE_TOKENS_INFLUX_TOKEN=..."
    warn "  export CLAUDE_TOKENS_INFLUX_ORG=... CLAUDE_TOKENS_INFLUX_BUCKET=..."
    warn "  export CLAUDE_TOKENS_HOST=..."
